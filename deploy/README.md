...
供 **完全無對外網路** 的環境安裝 `sftp_download` 及其相依套件之用。
...
sftp_download 使用**專屬虛擬環境**（與 radar / SHM 等其他專案慣例一致），
預設路徑：
...
~/venv/wanhai_nssms/share/sftp_download/bin/python deploy/health_check.py
...
```bash
# 啟用 venv 後執行
source ~/venv/wanhai_nssms/share/sftp_download/bin/activate
python main.py --cli

# 或不啟用、直接用 venv 的絕對路徑（適合排程 crontab）
~/venv/wanhai_nssms/share/sftp_download/bin/python \
    /home/mic-733ao/Documents/wanhai_nssms/share/sftp_download/main.py --cli
```

> 離線建立 venv 需要 `python3-venv`（ensurepip）；本機已具備，`python3 -m venv`
> 會用內建 wheel bootstrap pip，全程不需連網。
