...
為了正確偵測版本是否有變，本工具在**本地端儲存路徑**根目錄會建立一個隱藏的版本紀錄檔 `.sftp_download_manifest.json`，記錄每個檔案目前對應到來源端的檔案大小與修改時間——**下載一開始（尚未實際寫入資料前）就會先寫入這筆紀錄**，即使下載中途被中斷，下次重試時仍能正確比對出「這是同一版本尚未下載完的部分，可以安全接續」，而不是每次都得重新用猜的。之後每次執行都會拿遠端目前的檔案大小 + 修改時間，跟紀錄檔裡的值比對：

- **大小、修改時間都相同** → 判斷為未變更，略過。
- **沒有紀錄可比對，但本地端跟遠端大小剛好相同** → 判斷為未變更，略過，並從這次開始建立版本紀錄。
- **確認是同一版本（紀錄與目前遠端一致）、但本地檔案比遠端小** → 確定是尚未下載完成的同一版本，可以安全接續下載。
- **其餘情況** → 代表來源檔案已被更新，或是本地端有來路不明、無法確認是否為同一版本的資料（例如本地端本來就放了同名但內容其實不同的檔案）。**為避免把不相干的新舊內容硬接在一起造成檔案損毀，一律整份重新下載**，依 `duplicate_mode` 設定決定處理方式：

  - **`overwrite`（預設，GUI 進階選項的「直接覆蓋」）**：整份重新下載，直接覆蓋原檔名，不保留舊內容。
  - **`duplicate`（GUI 進階選項的「另存新檔」）**：保留舊檔不動，把新版本另存成新檔案，檔名規則是「原檔名 + `_` + `duplicate_suffix` 設定值」，第一次是 `原檔名_copy.ext`，同一個檔案再被更新則依序是 `原檔名_copy1.ext`、`原檔名_copy2.ext`……後綴字串可自訂。適合需要保留每一版歷史檔案的情境。

> 只有在**明確可以確認是同一版本**（紀錄檔比對相符）時才會接續下載；找不到紀錄、又無法單純用檔案大小判斷是否相符時，一律視為需要整份重新下載，不會冒險接續一份來源不明的本地檔案，以免下載出內容錯亂的檔案。「另存新檔」模式則更進一步：只要需要整份重新下載，就一定存成新檔案，斷點續傳形同停用（唯一例外是已確認同一版本、只是尚未下載完成的情況，這時「直接覆蓋」模式仍會接續下載，但「另存新檔」模式仍然選擇整份重新存成新檔）。

> 若某個檔案是**這台裝置第一次遇到、還沒有版本紀錄**（例如升級到這個版本之前就已經下載過的舊檔案，或本地端本來就已經放了同名檔案），沒有歷史資料可比對時，只有在「本地檔案跟遠端大小剛好相同」才會判斷為未變更略過；只要看得出大小不同，就會依上述 `duplicate_mode` 規則處理，不會因為缺乏歷史紀錄就忽略這個差異。
...
## 已知issue
- "直接覆蓋" 與 "斷點續傳" 若同時啟動，本地端檔案小於遠端檔案時會沒辦法成功覆蓋檔案